    ...
    for i in range(tourSum, addedTourSum):
      ...
      if existPlace == False:
        ...
        for j in range(len(raceRankDict[currentRaceRank])):
          sheet.cell(row=i,column=23+j).value = raceRankDict[currentRaceRank][j]
...
for i in range(len(jsonData)):
  ...
  placeTour = generate_boat_race.generate_one_day_race_result_list(sheet=sheet,jsonData=jsonData[stringIndex])
  ...
  generate_other_boat_data(placeTour=placeTour, reservedRaceRank=jsonData[stringIndex]["raceRankList"],roundIndexList=roundIndexList)
